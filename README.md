...
- **賽程與戰報（schedule）**：✅ 球隊、比分、狀態、球場的解析已對照官網
  ...
  `home_score` 因此正確維持 None）。**比賽日期目前還是抓不到**：第一版猜
  「class 名稱裡有 date/day」證實是錯的（抓到賽程輪次編號，不是日曆
  日期）已經拿掉；第二版改猜「內容形狀」，對照真實排程結果後確認：不管是
  卡片本身、還是往前 50 個元素內，都沒有任何日期形狀的文字，用「class 名稱
  裡有 schedule 字樣」去猜診斷用容器時還誤中了搜尋篩選表單（`ScheduleSearch`，
  字面符合但語意無關）。已經改成不再靠字串猜容器，直接照 DOM 結構從真正
  的卡片往上爬固定層數當診斷輸出，下次排程執行應該能看到卡片真正的祖先
  結構。**先發投手（`away_pitcher`／`home_pitcher`）目前確認官網賽程卡片
  本身沒有這項資訊**：拿抽樣到的下一場未開賽比賽（照 game_no 推斷最接近的
  一場）真實 HTML 對照過，`.remark` 底下除了幾個空的 Vue 條件渲染註解跟
  開賽時間之外沒有任何先發投手相關文字，比較可能是官網要等更接近比賽時間
  才公布、或這項資訊根本不在這個頁面上。程式維持「找不到就是 None、不影響
  其他欄位」，比賽勝率預測（見下面「比賽勝率預測」章節）在抓不到先發投手
  時會自動退回「只反映兩隊整體實力」，不會因此整個失敗。

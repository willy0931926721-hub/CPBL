...
- **投手數據（pitching）**：✅ 已對照真實結構修正並確認可用。跟打者共用
  ...
  驗證出錯會再對照修正）。
- **打者/投手榜單只顯示前 15 名**：⚠️ 觀察到目前匯出的 `batting.csv`／
  `pitching.csv` 都剛好只有 15 位球員，跟一整個球季實際有出賽紀錄的球員數
  差很多，猜測官網「全記錄查詢」榜單預設有分頁、只顯示前面一小部分。
  已經加上 `_try_expand_page_size()`（`scraper/http.py`）嘗試找「每頁筆數」
  下拉選單並切到「全部」，`batting.py` 也因此從純 requests 改成瀏覽器渲染
  （才能跟下拉選單互動）。**這部分還沒對照過官網真實 HTML 確認選單長什麼
  樣子**，觸發排程後如果 `cli.py` 印出「只抓到 15 筆，這剛好是常見的分頁
  預設筆數」這種警告，代表這次的猜測沒有命中，需要把實際榜單頁的 HTML
  診斷輸出貼回來才能對照修正。
- **賽程與戰報（schedule）**：✅ 已對照官網真實結構修正並確認可用（球隊、
  ...
  過去用「class 名稱裡有沒有 date/day」判斷，結果在雲端實際執行時誤抓到
  跟賽程輪次編號有關的元素，匯出的日期欄位整批變成 "1"、"2"、"3" 這種
  連續整數，不是真正的日曆日期——已經改成用「內容形狀」（長得像
  `2026/08/05`、`8/05`、`8月5日` 這種樣式的文字）去找，不管 class 實際
  叫什麼名字都能用，理論上比舊版更可靠，但同樣還沒有機會對照下一次雲端
  執行的真實結果驗證。**先發投手（`away_pitcher`／`home_pitcher`）**：
  ⚠️ 全新、純猜測性的欄位，目前完全沒有對照過官網「未開賽」比賽卡片的
  真實 HTML（開發環境連不到官網，只對照過「已完賽」卡片），抓不到就是
  `None`，不影響其他欄位；比賽勝率預測（見下面「比賽勝率預測」章節）
  找得到先發投手時會拿他的 ERA 做一個小幅度的調整，找不到就跟過去一樣
  只反映兩隊整體實力。

...
- **賽程與戰報（schedule）**：✅ 球隊、比分、狀態、球場的解析已對照官網
  真實結構確認可用。**比賽日期／先發投手則還沒抓到**：日期第一版猜
  「class 名稱裡有 date/day」，實際執行後證實是錯的（抓到賽程輪次編號
  "1"、"2"、"4"...不是日曆日期），已經拿掉；第二版改猜「內容形狀」
  （長得像 `2026/08/05`、`8/05`、`8月5日` 的文字），對照一次真實排程
  執行結果，這一版同樣沒有找到任何符合的文字——代表真正的日期標題可能用
  完全不同的格式呈現、或根本不在卡片附近的 DOM。先發投手（`away_pitcher`／
  `home_pitcher`）的猜測（`.pitcher.away`／`.pitcher.home`）目前也一樣
  沒有命中。兩者都改成「找不到就留空、不影響其他欄位」，且
  `fetch_schedule()` 現在會在整批比賽都抓不到日期／先發投手時，直接把
  賽程區塊（或第一場未開賽比賽卡片）的原始 HTML 印進 log，下次排程執行
  後把這段診斷輸出貼回來，就能照真實結構一次修正到位，不用再另外寫
  診斷腳本手動跑。比賽勝率預測（見下面「比賽勝率預測」章節）在抓不到
  先發投手時會自動退回「只反映兩隊整體實力」，不會因此整個失敗。
